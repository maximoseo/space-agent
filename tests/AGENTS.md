...
- `browser_window_persistence_test.mjs`: focused real-app browser-store regression coverage for persisted browser-window restoration, ensuring reload restores URLs plus geometry and minimized state from browser-local storage, clamps restored windows back onto the current viewport, reuses the same fit pass on live resize, keeps persisted snapshots in sync with those fitted bounds, and raises a browser window to the top when its iframe or desktop webview content surface receives direct focus
