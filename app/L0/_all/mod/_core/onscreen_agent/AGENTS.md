...
- `_core/web_browsing` currently uses the prompt seams in three narrow ways: one onscreen-only top-level `browser-manager` skill auto-loads for browser-window management, `browser-control` auto-loads once the page exports `browser:open`, one hook injects a brief `currently open web browsers` status block when browser windows are open, listing the public numeric browser ids plus their latest url and title, and a second hook injects `last interacted web browser` only when a remembered interacted browser is still open at prompt-build time, loading fresh simplified page content then rather than caching it at interaction time; those skills and prompt examples should rely on the top-level numeric-id `space.browser` helpers only, with no per-window handles or explicit `sync(...)` step, because open or state or navigation or inspection or interaction helpers already settle internally and mutations return fresh state snapshots; when those conditions are not met the owner-module hooks should add nothing
