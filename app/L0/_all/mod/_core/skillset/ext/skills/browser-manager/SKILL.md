...
transient
- `currently open web browsers` in `_____transient` lists `browser id|url|title`
...
- when at least one browser window is open, the full auto-loaded `browser-control` skill covers navigation, page reads, refs, clicks, typing, and history actions
